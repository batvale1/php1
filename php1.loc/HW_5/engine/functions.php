 ...
 */
function prepareVariables($page, $getParams = [])
{
    $layout = 'main';
    $params = ['login' => 'admin'];
    switch ($page) {
        case 'index':
            ...
            break;
        case 'gallery':
            $params['gallery'] = getGallery($getParams['navpage']);
            $layout = 'gallery';
            break;
        case 'detail_img':
            incrementViews($getParams['img_id']);
            $params['detail_img_data'] = getGalleryImg($getParams['img_id']);
            $layout = 'detail_img';
            break;
        case 'catalog':
            $params['catalog'] = [
...
function my_var_dump($data, $label='', $return = false) {
